...
mongoose.connect(process.env.MONGODB_URI || "mongodb://localhost:27017/dealershipsDB");
...
app.get('/fetchReviews/dealer/:id', async (req, res) => {
  try {
    ...
    const documents = await Reviews.find({ dealership: Number.isNaN(dealershipId) ? req.params.id : dealershipId });
    res.json(documents);
  } catch (error) {
...
app.get('/fetchDealer/:id', async (req, res) => {
    ...
    try {
        ...
        const dealer = await Dealerships.findOne({ id: Number.isNaN(numericId) ? id : numericId });
        res.json(dealer);
    } catch (err) {
        console.error(err);
...
app.post('/insert_review', express.json(), async (req, res) => {
  ...
  const new_id = documents.length ? Number(documents[0].id) + 1 : 1;
